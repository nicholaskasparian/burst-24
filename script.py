...
for i in tradeable:
    time.sleep(0.1)
    print(i)
    url = f"https://data.alpaca.markets/v2/stocks/bars?symbols={i}&timeframe=1day&start={yesterday}&end={yesterday}&limit=1&adjustment=raw&feed=sip&sort=asc"


    headers = {
        "accept": "application/json",
        "APCA-API-KEY-ID": f"{APCA_API_KEY_ID}",
        "APCA-API-SECRET-KEY": f"{APCA_API_SECRET_KEY}"
    }       

    response = requests.get(url, headers=headers)
    lPrice = 12
    price = 12
    percentageChange = (price / lPrice - 1) * 100
